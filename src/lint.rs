...
fn is_fully_anchored(pattern: &str) -> bool {
    ...
    let ends_anchored = ends_with_unescaped(pattern, '$') || pattern.ends_with("\\z");
    starts_anchored && ends_anchored
}

/// Returns `true` when `pattern` ends with `terminator` and that character is not itself
/// escaped. A pattern such as `^price: \$` ends in a *literal* dollar and is therefore not
/// anchored, so the trailing backslash run is counted for parity.
fn ends_with_unescaped(pattern: &str, terminator: char) -> bool {
    let Some(head) = pattern.strip_suffix(terminator) else {
        return false;
    };
    head.chars().rev().take_while(|c| *c == '\\').count() % 2 == 0
}
...
mod tests {
    ...
    #[test]
    fn treats_an_escaped_trailing_dollar_as_unanchored() {
        // `^price: \$` ends in a literal dollar, so it can still match with trailing content
        // and the warning must not be suppressed.
        let mut options = LintOptions::default();
        options.message_pattern = Some(build_message_pattern("^price: \\$", None).unwrap());

        let outcome = lint_message("price: $100 extra", &options);
        assert_eq!(
            outcome.warnings_before,
            vec![
                "message pattern `^price: \\$` is not anchored, so it matches anywhere in the title line; \
                 add `^` and `$` to require a full-title match"
                    .to_string()
            ]
        );
