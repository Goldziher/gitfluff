...
- `git revert`, `fixup!`, `squash!` and `amend!` commits were rejected by the default preset; they
  are now skipped. The skip keys on the generated subject rather than on an in-progress rebase, so
  a message reworded during `git rebase -i` is still linted.
...
- The npm installer left a rejected archive in `bin/` when verification failed, and a connection
  drop mid-download crashed `postinstall` with an unhandled stream error instead of a readable
  message. The archive is now always cleaned up and response errors are surfaced properly.
- A `[rules.message]` config printed its "replaces the spec check" notice on every invocation,
  including passing ones; it is now shown only when the lint rejects the message.

### Security

- The npm wrapper's `tar` and `adm-zip` dependencies are upgraded to `^7.5.22` and `^0.6.1`,
  clearing a critical `node-tar` decompression advisory and several high-severity advisories in
  the archive extractors. `npm audit` is clean.
