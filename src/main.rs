...
const AI_CLEANUP_RULES: &[(&str, &str, &str)] = &[
    ...
    // Both lines are spelled out so nothing between them can be swallowed.
    (
    ...
    // The trailer is optionally followed by a line holding only the bare `<email>`.
    (
...
/// (merge, revert, cherry-pick, rebase). Those messages are not hand authored, so linting
/// them only blocks the operation.
const GIT_SEQUENCER_MARKERS: &[&str] = &[
    "MERGE_HEAD",
    "REVERT_HEAD",
    "CHERRY_PICK_HEAD",
    "rebase-merge",
